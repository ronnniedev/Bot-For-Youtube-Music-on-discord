"""
    ...
    -2º Reproduce el sonido en el canal donde se encuentra el usuario,
     en caso de no encontrarse en el lanza un mensaje de aviso
    -3º Se borra el arvhivo alojado en el sistema, esto hace que
    el bot sea compatible con uso en la nube ya que
    no sobrecarga el sistema donde se encuentra con archivos
...
"""
    Preparacion de la configuracion del reproductor, 
    en formato dejamos puesto el bestAudio para que 
    el bot recoga el formato con mejor claidad
"""
...
class YTDLSource(discord.PCMVolumeTransformer):
    """
        Clase que conforma la configuracion del descargador de archivos
    """
...
async def reproducir_audio(ctx,url,client,guild_id):
    ...
    if ctx.voice_client:
        ...
        if voice.is_playing():
           await ctx.send("Estoy reproduciendo un audio, espera a que "
                          "termine o usa el comando -->!stop para que acabe la cancion actual")
        else:
...
async def after_playing(ctx, filename,guild_id,client):
    """
        Subrutina llamada tras la reproduccion de audio, elimina
        el archivo alojado con el nombre proporcionado y
        en caso de que la cola no halla terminado
        reproduce la siguiente cancion, si no termina la cola
        @param: ctx context
    ...
    """
    global en_cola
    antiguo_filename = filename
    try:
        os.remove(antiguo_filename)
    except Exception:
        pass
...
async def skip(ctx,client):
    """
        Skipea el audio dentro de la cola, si no hay
        una url alojada en el diccionario el bot se desconecta
        @param: ctx context
        @param: client
    """
    global cola
    ...
    if ctx.voice_client:
        ...
        if voice.is_playing():
            """
                En cuanto se haga stop el reproductor cambiara el filename
                por lo que guardamos el filename antiguo
            """
            voice.stop()
        else:
...
async def stop(ctx,client):
    """
        Si el audio se esta reproduciendo este es cortado
        y el bot se va del canal, si no y solo esta el bot
        en espera simplemente se va.
        @param: ctx context
        @param: client
    """
    global cola
    ...
    if(ctx.voice_client):
        ...
        if voice.is_playing():
            voice.stop()
            await ctx.voice_client.disconnect()
            # esto lo usamos para evitar el error de acceso,
            # al esperar 2 segundos le damos al bot tiempo a cerrar el proceso
            cola[guild_id] = []
            en_cola = False
        else:
            await ctx.voice_client.disconnect()
